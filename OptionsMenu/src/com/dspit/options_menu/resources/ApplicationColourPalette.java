

package com.dspit.options_menu.resources;

import javafx.scene.paint.Color;

/**
 * An Interface which allows stores default colours the 
 * application will be using.
 * 
 * @author David Boivin (Spit)
 */
public interface ApplicationColourPalette {
	
	public static final Color BACKGROUND = Color.web("#A9A9A9");
	public static final Color COMPONENT_BACKGROUND = Color.web("#FF8800");
	public static final Color COMPONENT_FORGROUND = Color.web("#FFFFFF");
	public static final Color ACCENT_BACKGROUND = Color.web("#FFBB33");
	public static final Color ACCENT_FORGROUND = Color.web("#FFFFFF");
	
}
